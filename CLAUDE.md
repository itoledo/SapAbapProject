...
2. **MCP server** (`SapAbapProject.McpServer`) that exposes the same SAP-querying capabilities as MCP tools for LLM agents (Claude Desktop, VS Code Copilot). Both rendered ABAP source and structured JSON metadata are returned per tool call.
...
- **`SapAbapProject.RfcExtractor`** (`net472;net8.0`) — All SAP RFC logic. Depends on `SapNwRfc` (managed wrapper around the native `sapnwrfc.dll`). Key components:
  - `AbapObjectExtractor` — Top-level `IAbapExtractor` implementation; lazily opens a single `SapConnection`, dispatches to per-type extractors. Serializes ALL operations through a `SemaphoreSlim(1,1)` so the single SAP connection is never accessed concurrently. Exposes single-object lookups (`GetFunctionModuleAsync(name)`, `GetTableAsync(name)`, etc.), discovery (`ListFunctionModulesAsync`, `ListTablesAsync`), and a generic `ReadTableAsync` escape hatch for ad-hoc `RFC_READ_TABLE` queries — these were added so the MCP server can be a thin wrapper.
...
- **`SapAbapProject.McpServer`** (`net8.0`, console app, output: `sap-mcp.exe`) — MCP server over stdio transport. Built on the official `ModelContextProtocol` SDK (decorator-based: `[McpServerToolType]` + `[McpServerTool]`) and `Microsoft.Extensions.Hosting`. Three layers:
  ...
  - `SapTools` — All MCP tools live here. Each tool method is `[McpServerTool, Description("...")]` with `[Description]` on every parameter so the LLM gets a full schema. Object-detail tools return `SapObjectResponse` (rendered ABAP `Source` + structured `Metadata`); list tools return `SapObjectSummaryResponse`. `sap_read_table` exposes the generic `RFC_READ_TABLE` escape hatch — intentionally unrestricted because the server is targeted at dev SAP systems.
